...
const BlogsCard = ({ image, title, description, author, date }) => {
  return (
    <Link to={`/blogs/${title}`} className='cursor-pointer' onClick= {
      () => window.scrollTo(0, 0)
    } state={{image, title, description, author, date}}>
      <div className='p-4 shadow-lg transition-all duration-500 hover:shadow-xl'>
        <div className='overflow-hidden'>
          <img src={image} alt={title} className='mx-auto h-[250px] w-full object-cover transition-all duration-700 hover:skew-x-2 hover:scale-110' />
        </div>
